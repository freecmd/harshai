...
from predictive_core import (
    ...
    lstm_residual_only_forecast,
    compare_models,
)
from anomaly_detection import detect_anomalies, compute_volatility
from sentiment_engine import compute_news_sentiment, compute_youtube_fame_score, compute_investor_sentiment_score
from hmcd_engine import (
    HMCDWeights, 
    compute_predictive_strength, 
    compute_technical_confirmation, 
    compute_risk_score, 
    compute_investor_sentiment,
    hmcd_decision
)
...
# Page configuration
st.set_page_config(
    page_title="A Multi- Modal Framework Employing HMCD for Market Prediction and Deep learning (LSTM-Prophet) for Predictive Analysis.",
    page_icon="📊",
    layout="wide",
...
def sanitize_df_for_streamlit(df: object) -> pd.DataFrame:
    """Return a DataFrame safe for Arrow serialization and st.dataframe/st.table.
    Accepts DataFrame, dict, or other objects; always returns a DataFrame.
    """
    try:
        if df is None:
            return pd.DataFrame()
        ...
        try:
            return cast(pd.DataFrame, df).astype(str)
        except Exception:
...
if OPENAI_API_KEY and openai is not None:
    try:
        # Auto-detect OpenRouter if no base_url provided
        ...
        try:
            ...
            openai_client = _OpenAI(api_key=OPENAI_API_KEY, base_url=OPENAI_BASE_URL)
        except Exception:
            # Fallback to attribute on module
            try:
                openai_client = openai.OpenAI(api_key=OPENAI_API_KEY, base_url=OPENAI_BASE_URL)  # type: ignore
            except Exception:
                pass
    except Exception:
        ...
        try:
            ...
            if OPENAI_BASE_URL:
                try:
                    setattr(openai, "api_base", OPENAI_BASE_URL)
                except Exception:
                    pass
        except Exception:
            pass
...
if GOOGLE_GEMINI_API_KEY and genai is not None:
    try:
        # 1. Configure the API key (if supported by installed genai version)
        try:
            if callable(getattr(genai, "configure", None)):
                genai.configure(api_key=GOOGLE_GEMINI_API_KEY)  # type: ignore
            else:
                raise AttributeError("configure not available")
        except (AttributeError, TypeError):
            # Older versions: set env variable + fallback attribute
            os.environ["GOOGLE_GEMINI_API_KEY"] = GOOGLE_GEMINI_API_KEY
            if hasattr(genai, "api_key"):
                try:
                    genai.api_key = GOOGLE_GEMINI_API_KEY  # type: ignore
                except Exception:
                    pass

        # 2. Try primary model
        if hasattr(genai, "GenerativeModel") and callable(getattr(genai, "GenerativeModel", None)):
            try:
...
class MarketInsightsDashboard:
    ...
    def predict_price_arima(_self, hist_df, steps=5, order=(5,1,0), return_ci: bool = True):
        if ARIMA is None:
            return None, "ARIMA not available"
        try:
            forecast = arima_forecast(hist_df, steps=steps, order=tuple(order) if order else (5, 1, 0))
    ...
    @st.cache_data(ttl=1800)
    def predict_price_hybrid(_self, hist_df, steps=5, lookback: int = 20, epochs: int = 10):
        """Hybrid LSTM–Prophet residual model (paper requirement)."""
        try:
            prophet_fc, res_fc, hybrid_fc = hybrid_lstm_prophet_forecast(
                hist_df,
                steps=steps,
                lookback=int(lookback or 20),
                epochs=int(epochs or 10),
            )
            return (prophet_fc, res_fc, hybrid_fc), None
        except Exception as e:
            return None, str(e)
    ...
    def analyze_sentiment(self, text):
        try:
            ...
            textblob_polarity = float(getattr(sentiment, "polarity", 0.0))
            textblob_subjectivity = float(getattr(sentiment, "subjectivity", 0.0))
            return {
...
def main():
    st.title("A Multi- Modal Framework Employing HMCD for Market Prediction and Deep learning (LSTM-Prophet) for Predictive Analysis")
    st.markdown("---")
    ...
    with st.sidebar.expander("📈 Forecasts"):
        ...
        st.session_state.use_lstm = st.checkbox(
            "LSTM residual forecast", value=st.session_state.get("use_lstm", False)
        )
        st.session_state.use_hybrid = st.checkbox(
            "Hybrid LSTM–Prophet", value=st.session_state.get("use_hybrid", True)
        )
    ...
    with st.sidebar.expander("🤖 AI Options"):
        ...
        st.session_state.use_news_sentiment = st.checkbox(
            "Use News Sentiment (C2)", value=st.session_state.get("use_news_sentiment", True)
        )
        st.session_state.use_youtube_fame = st.checkbox(
            "Use YouTube Fame (C2)", value=st.session_state.get("use_youtube_fame", True)
        )
    ...
    if analysis_type == "Screener":
        ...
        st.session_state.screener_rsi = st.sidebar.slider("RSI range", 5, 95, value=tuple(st.session_state.get('screener_rsi', (30, 70)))) # type: ignore
    ...
    if st.sidebar.button("🚀 Analyze", type="primary"):
        with st.spinner("Fetching data..."):
            ...
            if stock_data and stock_data.get('current_price') is not None:
                ...
                # Store model outputs for individual frames
                lstm_preds = None
                prophet_frame = None
                ...
                # Compute HMCD Decision BEFORE displaying tabs
                try:
                    hist_df_hmcd = stock_data.get('history')
                    if hist_df_hmcd is not None and not hist_df_hmcd.empty:
                        close = hist_df_hmcd['Close']
                        ...
                        # Predictive strength from evaluation (Hybrid metrics)
                        pred_strength = None
                        try:
                            eval_metrics = compare_models(
                                hist_df_hmcd,
                            ...
                            m = eval_metrics.get("Hybrid")
                            if m is not None:
                                pred_strength = compute_predictive_strength({
                                    "rmse": m.rmse,
                                    "mae": m.mae,
                        ...
                        # Investor sentiment (News + YouTube Fame) with ablation
                        use_news = bool(st.session_state.get("use_news_sentiment", True))
                        use_yt = bool(st.session_state.get("use_youtube_fame", True))
                        news_sent = compute_news_sentiment(st.session_state.get('news_data') or [])
                        fame_score = compute_youtube_fame_score(st.session_state.get('youtube_data') or [])
                        
                        # Use new compute_investor_sentiment from hmcd_engine
                        # Handle None values from sentiment functions
                        news_val = news_sent if (use_news and news_sent is not None) else 0.5
                        fame_val = fame_score if (use_yt and fame_score is not None) else 0.5
                        investor_sent = compute_investor_sentiment(
                            news_sentiment=news_val,
                            youtube_fame=fame_val
                        )

                        # Risk/Anomaly score
                        anom_out = detect_anomalies(hist_df_hmcd, contamination=0.03)
                        last_anom = float(anom_out.frame['anomaly_score'].iloc[-1])
                        vol = compute_volatility(close)
                        risk_score = compute_risk_score(last_anom, vol)

                        # Technical confirmation
                        tech_score = compute_technical_confirmation(
                            rsi_last,
                            float(macd.iloc[-1]) if len(macd) else 0.0,
                            float(macd_signal.iloc[-1]) if len(macd_signal) else 0.0,
                        )

                        # Prepare individual scores
                        pred_strength_val = float(pred_strength) if pred_strength is not None else 0.5
                        investor_sent_val = float(investor_sent) if investor_sent is not None else 0.5
                        risk_val = float(risk_score)
                        tech_val = float(tech_score)
                        
                        # Call new HMCD decision function
                        weights = HMCDWeights()
                        hmcd_result = hmcd_decision(
                            predictive_strength=pred_strength_val,
                            technical_confirmation=tech_val,
                            investor_sentiment=investor_sent_val,
                            risk_penalty=risk_val,
                            weights=weights
                        )
                        
                        # Store for tab display
                        st.session_state.hmcd_last = {
                            "score": hmcd_result.score,
                            "decision": hmcd_result.decision,
                            "rationale": hmcd_result.rationale,
                            "confidence": hmcd_result.confidence,
                            "contributions": hmcd_result.contributions,
                            "signals": hmcd_result.signals_breakdown,
                            "anomaly_score": last_anom,
                        }
                except Exception as e:
                    # Clear HMCD if computation fails
                    st.session_state.hmcd_last = None
                ...
                # For LSTM/Hybrid models, ensure we have enough data
                if hist_df is not None and not hist_df.empty:
                    lookback = int(st.session_state.get('lstm_lookback', 20))
                    min_required = lookback + 20
                    
                    if (st.session_state.get('use_lstm') or st.session_state.get('use_hybrid')) and len(hist_df) < min_required:
                        # Fetch more data for modeling purposes
                        longer_period = "3mo" if period in ["1d", "5d", "1mo"] else period
                        try:
                            extended_data = dashboard.get_stock_data(symbol, longer_period)
                            if extended_data and 'history' in extended_data and len(extended_data['history']) >= min_required:
                                hist_df = extended_data['history']
                        except Exception:
                            pass  # Use original data if extended fetch fails
                ...
                if hist_df is not None and not hist_df.empty and price_chart is not None:
                            ...
                            if fc is not None:
                                try:
                                    if isinstance(fc, tuple) and len(fc) == 2:
                                        ...
                                        yvals = np.asarray(getattr(fc, "values", fc))
                                        price_chart.add_trace(
                                            go.Scatter(x=future_idx, y=yvals[:len(future_idx)], name='ARIMA Forecast', line=dict(color='orange', dash='dash'))
                                        )
                                except Exception:
                    ...
                    if st.session_state.get('use_lstm'):
                        try:
                            ...
                            if preds is not None:
                                try:
                                    lstm_preds = preds if isinstance(preds, list) else list(preds)
                                    price_chart.add_trace(
                                        go.Scatter(x=future_idx, y=lstm_preds[:len(future_idx)], name='LSTM Forecast', line=dict(color='purple', dash='dot'))
                                    )
                                except Exception:
                                    pass
                            elif err:
                            ...
                            st.caption(f"LSTM: {e}")
                    # Hybrid LSTM–Prophet
                    if st.session_state.get('use_hybrid'):
                        try:
                            hybrid_res, err = dashboard.predict_price_hybrid(
                                hist_df,
                                steps=steps,
                                lookback=int(st.session_state.get('lstm_lookback', 20)),
                                epochs=int(st.session_state.get('lstm_epochs', 10)),
                            ...
                            if hybrid_res is not None:
                                try:
                                    if isinstance(hybrid_res, tuple) and len(hybrid_res) == 3:
                                        _, _, hybrid_fc = hybrid_res
                                        hybrid_forecast = hybrid_fc
                                        hybrid_vals = hybrid_fc.values.tolist() if hasattr(hybrid_fc, 'values') else list(hybrid_fc)
                                        price_chart.add_trace(
                                            go.Scatter(x=future_idx, y=hybrid_vals[:len(future_idx)], name='Hybrid LSTM–Prophet', line=dict(color='teal', dash='solid'))
                                        )
                                    else:
                                        raise ValueError(f"Expected tuple of 3, got {type(hybrid_res)}")
                                except Exception:
                                    pass
                            elif err:
                                hybrid_err = err
                                st.caption(f"Hybrid: {err}")
                        except Exception as e:
                            hybrid_err = str(e)
                            st.caption(f"Hybrid: {e}")
                    ...
                    if st.session_state.get('use_prophet'):
                        try:
                            ...
                            prophet_err = str(e)
                            st.caption(f"Prophet: {e}")

                if price_chart:
                    st.plotly_chart(price_chart, use_container_width=True)
                    # Separate frames for individual views
                    tabs = st.tabs(["LSTM Forecast", "Prophet Forecast", "Technical Indicators", "HMCD Decision"])
                    with tabs[0]:
                        if lstm_preds is not None and hist_df is not None and future_idx is not None:
                            st.subheader("LSTM Residual Forecast")
                            fig_lstm = go.Figure()
                            fig_lstm.add_trace(go.Scatter(x=future_idx, y=list(lstm_preds)[:len(future_idx)], name='LSTM Residual Forecast'))
                            st.plotly_chart(fig_lstm, use_container_width=True)
                        elif lstm_err:
                            st.error(f"LSTM forecast failed: {lstm_err}")
                        else:
                            st.info("LSTM residual forecast not available. Enable LSTM forecast or run Analyze.")
                    with tabs[1]:
                        if prophet_frame is not None:
                            st.subheader("Prophet Forecast")
                            fig_prophet = go.Figure()
                            fig_prophet.add_trace(go.Scatter(x=pd.to_datetime(prophet_frame['ds']), y=prophet_frame['yhat'], name='Prophet'))
                            st.plotly_chart(fig_prophet, use_container_width=True)
                        elif prophet_err:
                            st.error(f"Prophet forecast failed: {prophet_err}")
                        else:
                            st.info("Prophet forecast not available. Enable Prophet trend or run Analyze.")
                    with tabs[2]:
                        if hist_df is not None and not hist_df.empty:
                            st.subheader("Technical Indicator Analysis")
                            close = hist_df['Close']
                            rsi_val = float(_rsi(close).iloc[-1]) if len(close) else 50.0
                            ema12 = _ema(close, 12)
                            ema26 = _ema(close, 26)
                            macd = ema12 - ema26
                            macd_signal = _ema(macd, 9)
                            ...
                            c2.metric("MACD", f"{float(macd.iloc[-1]):.4f}")
                            c3.metric("Signal", f"{float(macd_signal.iloc[-1]):.4f}")
                        else:
                            st.info("Technical indicators unavailable. Run Analyze first.")
                    with tabs[3]:
                        hmcd_last = st.session_state.get("hmcd_last")
                        if isinstance(hmcd_last, dict):
                            st.subheader("🧭 HMCD Decision (Balanced Multi-Criteria)")
                            c1, c2, c3, c4 = st.columns(4)
                            with c1:
                                st.metric("HMCD Score", f"{hmcd_last.get('score', 0.0):.3f}")
                            with c2:
                                decision = hmcd_last.get("decision", "HOLD")
                                decision_color = "🟢" if decision == "BUY" else "🔴" if decision == "SELL" else "🟡"
                                st.metric("Decision", f"{decision_color} {decision}")
                            with c3:
                                st.metric("Confidence", f"{hmcd_last.get('confidence', 0.0):.1%}")
                            with c4:
                                st.metric("Risk Score", f"{hmcd_last.get('anomaly_score', 0.0):.3f}")
                            
                            st.caption(f"**Rationale**: {hmcd_last.get('rationale', '')}")
                            
                            # Show contribution breakdown
                            with st.expander("📊 Contribution Breakdown"):
                                contrib = hmcd_last.get('contributions', {})
                                if contrib:
                                    contrib_df = pd.DataFrame([
                                        {"Criterion": k.replace('_', ' ').title(), "Contribution": v}
                                        for k, v in contrib.items()
                                        if k not in ["raw_score", "final_score"]
                                    ])
                                    st.dataframe(contrib_df, use_container_width=True)
                                
                                # Show signal interpretations
                                signals = hmcd_last.get('signals', {})
                                if signals:
                                    st.markdown("**Signal Interpretations:**")
                                    for criterion, interpretation in signals.items():
                                        st.caption(f"• {criterion.title()}: {interpretation}")
                        else:
                            st.info("HMCD decision not available. Run Analyze to compute it.")
                    ...
                    try:
                        ...
                        pass
                    # Evaluation & Validation (RMSE, MAE, Directional Accuracy)
                    with st.expander("📏 Model Evaluation (ARIMA vs Prophet vs LSTM vs Hybrid)"):
                        if st.button("Run evaluation", key="run_eval"):
                            try:
                                hist_df_local = stock_data.get('history')
                                ...
                                st.dataframe(sanitize_df_for_streamlit(pd.DataFrame(rows)))
                            except Exception as e:
            ...
            if analysis_type in ["YouTube Trends", "Stock Analysis"]:
                ...
                if youtube_data:
                    ...
                    st.session_state.youtube_data = demo_youtube
...
def render_technicals_section(symbol: str, stock_data: dict | None):
    ...
    fig.add_hrect(y0=30, y1=70, line_width=0, fillcolor="lightgray", opacity=0.2, row=2, col=1)  # type: ignore[arg-type]
...
def _backtest_sma(df: pd.DataFrame, fast: int, slow: int, fee_bps: float, initial: float = 10000) -> dict:
    ...
    # Trades
    entries = (pos.diff() > 0) # type: ignore
    exits = (pos.diff() < 0) # type: ignore
    trades = []
    in_trade = False
    entry_price = None
...
def render_anomaly_detection(symbol: str, stock_data: dict | None):
    ...
    recent = out.frame.tail(120)
    anom = recent[recent['anomaly_flag']]
    ...
    with c2:
        ...
        if not anom.empty:
            st.dataframe(sanitize_df_for_streamlit(anom[['Close', 'anomaly_score']].tail(10)))
...
def render_market_ripple_engine(symbol: str):
    ...
    for i in corr.columns:
        for j in corr.columns:
            if i < j:
                # Use .at for faster scalar access and avoiding type warnings about Series/DataFrame rets
                w = float(corr.at[i, j])  # type: ignore[arg-type]
                if abs(w) >= 0.45:
